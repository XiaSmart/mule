...
public final class ConnectionProviderDefinitionParser extends AbstractDefinitionParser
{
    ...
    protected void doParse(Builder definition) throws ConfigurationException
    {
        definition.withIdentifier(hyphenize(providerModel.getName()))
                ...
                .withConstructorParameterDefinition(fromFixedValue(getConnectionManager()).build())
    ...
    private ConnectionManagerAdapter getConnectionManager() throws ConfigurationException
    {
        if (connectionManagerAdapter == null)
        {
            try
            {
                connectionManagerAdapter = muleContext.getRegistry().lookupObject(ConnectionManagerAdapter.class);
            }
