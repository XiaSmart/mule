...
/**
 * Wrapper for {@link DataTypeFactory#dataTypeBuilder()} to use form Spring parsers.
 */
final class DataTypeFactoryBean implements FactoryBean<DataType>
{
    ...
    {
        final DataTypeBuilder<?> builder = dataTypeBuilder(type);
        if (mimeType != null)
        {
            builder.forMimeType(mimeType);
        }
        if (encoding != null)
        {
