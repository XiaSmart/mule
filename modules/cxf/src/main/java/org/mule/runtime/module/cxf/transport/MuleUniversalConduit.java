...
public class MuleUniversalConduit extends AbstractConduit
{
    ...
    protected InputStream getResponseBody(Message m, MuleEvent result) throws TransformerException, IOException
    {
        ...
        if (response)
        {
            ...
            InputStream is = (InputStream) result.getMuleContext().getTransformationService().transform(result.getMessage(), INPUT_STREAM_DATA_TYPE).getPayload();
            PushbackInputStream pb = new PushbackInputStream(is);
            result.setMessage(new DefaultMuleMessage(pb, result.getMessage(), result.getMuleContext(), XML_STRING));
