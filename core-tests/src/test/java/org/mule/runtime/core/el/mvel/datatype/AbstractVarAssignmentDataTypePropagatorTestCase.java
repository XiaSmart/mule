...
public abstract class AbstractVarAssignmentDataTypePropagatorTestCase extends AbstractMuleContextTestCase
{
    ...
    {
        DataType<String> expectedDataType = dataTypeBuilder(String.class).forMimeType(JSON).withEncoding(CUSTOM_ENCODING).build();
    ...
    {
        final DataType expectedDataType = dataTypeBuilder(Map.class).forMimeType(UNKNOWN).withEncoding(CUSTOM_ENCODING).build();
        ...
        dataTypePropagator.propagate(testEvent, new TypedValue(propertyValue, STRING_DATA_TYPE), compiledExpression);
