...
public class MuleRegistryHelperTransformersAsObjectsTestCase extends AbstractMuleTestCase
{
    ...
    private static final DataType<Apple> APPLE_DATA_TYPE = dataTypeBuilder(Apple.class).build();
    ...
    private final Converter stringToApple = new MockConverterBuilder().from(STRING_DATA_TYPE).to(APPLE_DATA_TYPE).build();
    private final Converter appleToString = new MockConverterBuilder().from(APPLE_DATA_TYPE).to(STRING_DATA_TYPE).build();

    @Before
    public void setUp() throws Exception
    {
        TransformerResolver transformerResolver = mock(TransformerResolver.class);
        when(transformerResolver.resolve(STRING_DATA_TYPE, APPLE_DATA_TYPE)).thenReturn(stringToApple);
        when(transformerResolver.resolve(APPLE_DATA_TYPE, STRING_DATA_TYPE)).thenReturn(appleToString);
    ...
    {
        Transformer transformer1 = muleRegistryHelper.lookupTransformer(STRING_DATA_TYPE, APPLE_DATA_TYPE);
        Transformer transformer2 = muleRegistryHelper.lookupTransformer(APPLE_DATA_TYPE, STRING_DATA_TYPE);
