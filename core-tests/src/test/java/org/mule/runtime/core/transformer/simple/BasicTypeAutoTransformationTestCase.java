...
public class BasicTypeAutoTransformationTestCase extends AbstractMuleContextTestCase
{
    ...
    {
        return muleContext.getRegistry().lookupTransformer(STRING_DATA_TYPE, dataTypeBuilder(to).build());
    }

    private Transformer lookupToStringTransformer(Class from) throws TransformerException
    {
        return muleContext.getRegistry().lookupTransformer(dataTypeBuilder(from).build(), STRING_DATA_TYPE);
    }
