...
public class MuleRegistryHelperTransformerLookupTestCase extends AbstractMuleTestCase
{
    ...
    private static final DataType<Orange> ORANGE_DATA_TYPE = dataTypeBuilder(Orange.class).build();
    ...
    private final Converter stringToOrange = new MockConverterBuilder().from(STRING_DATA_TYPE).to(ORANGE_DATA_TYPE).build();
    private final Converter orangeToString = new MockConverterBuilder().from(ORANGE_DATA_TYPE).to(STRING_DATA_TYPE).build();

    @Before
    public void setUp() throws Exception
    {
        TransformerResolver transformerResolver = mock(TransformerResolver.class);
        when(transformerResolver.resolve(STRING_DATA_TYPE, ORANGE_DATA_TYPE)).thenReturn(stringToOrange);
        when(transformerResolver.resolve(ORANGE_DATA_TYPE, STRING_DATA_TYPE)).thenReturn(orangeToString);
    ...
    {
        Transformer transformer1 = muleRegistryHelper.lookupTransformer(STRING_DATA_TYPE, ORANGE_DATA_TYPE);
        Transformer transformer2 = muleRegistryHelper.lookupTransformer(ORANGE_DATA_TYPE, STRING_DATA_TYPE);
    ...
    {
        List<Transformer> transformers = muleRegistryHelper.lookupTransformers(STRING_DATA_TYPE, ORANGE_DATA_TYPE);
