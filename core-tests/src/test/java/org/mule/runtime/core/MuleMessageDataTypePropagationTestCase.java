...
import static org.mule.runtime.api.metadata.MimeTypes.ANY;
...
public class MuleMessageDataTypePropagationTestCase extends AbstractMuleTestCase
{
    ...
    public void setsDataTypeFromPreviousMessageOnCreation() throws Exception
    {
        ...
        DataType dataType = dataTypeBuilder(Long.class).forMimeType(APPLICATION_XML).withEncoding("UTF-16").build();
    ...
    public void setsNullPayloadWithDataType() throws Exception
    {
        ...
        DataType<Integer> newDataType = dataTypeBuilder(Integer.class).forMimeType(APPLICATION_XML).withEncoding(CUSTOM_ENCODING).build();
    ...
    public void updatesTypeOnTransformation() throws Exception
    {
        ...
        DataType outputDataType = dataTypeBuilder(Integer.class).forMimeType(ANY).build();
    ...
    public void updatesEncodingOnTransformation() throws Exception
    {
        ...
        Transformer transformer = mock(Transformer.class);
        when(transformer.isSourceDataTypeSupported(org.mockito.Matchers.<DataType<?>>any())).thenReturn(true);
        DataType outputDataType = dataTypeBuilder(Integer.class).withEncoding(CUSTOM_ENCODING).build();
        when(transformer.getReturnDataType()).thenReturn(outputDataType);
    ...
    public void maintainsDataTypeSubClassTypeTransformation() throws Exception
    {
        ...
        DataType originalDataType = dataTypeBuilder(RuntimeException.class).forMimeType(ANY).build();
        muleMessage.setDataType(originalDataType);

        Transformer transformer = mock(Transformer.class);
        DataType outputDataType = dataTypeBuilder(Object.class).forMimeType(ANY).build();
        when(transformer.getReturnDataType()).thenReturn(outputDataType);
        when(transformer.isSourceDataTypeSupported(dataTypeBuilder(muleMessage.getPayload().getClass()).build())).thenReturn(true);
    ...
    public void updatesMimeTypeOnTransformation() throws Exception
    {
        ...
        Transformer transformer = mock(Transformer.class);
        when(transformer.isSourceDataTypeSupported(org.mockito.Matchers.<DataType<?>>any())).thenReturn(true);
        DataType outputDataType = dataTypeBuilder(Integer.class).forMimeType(APPLICATION_XML).build();
        when(transformer.getReturnDataType()).thenReturn(outputDataType);
    ...
    {
        DataType<Integer> dataType = dataTypeBuilder(Integer.class).build();
    ...
    {
        DefaultMuleMessage muleMessage = new DefaultMuleMessage(TEST, muleContext);
        DataType<String> dataType = dataTypeBuilder(String.class).forMimeType(APPLICATION_XML).withEncoding(CUSTOM_ENCODING).build();
    ...
    {
        assertDataType(muleMessage, String.class, MimeTypes.ANY, DEFAULT_ENCODING);
    }
